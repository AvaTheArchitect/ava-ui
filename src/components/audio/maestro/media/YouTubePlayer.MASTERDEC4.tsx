...
/**
 * YouTube Player Component - December 4th, 2025 - V96.2
 * 
 * 🆕 UPDATES IN V96.2:
 * ✅ Added onStateChange prop to pass YouTube state to parent
 * ✅ YouTube state changes now properly sync with React state
 * ✅ Fixes play/pause button UI and cursor sync
 * 
 * 🔒 PRESERVED:
 * ✅ Works with external playerMode
 * ✅ Drift sync only when paused AND drift > 10s
 * ✅ forwardRef for parent access to YouTube methods
 * ✅ Time updates, play/pause state changes, offset support
 */
...
interface YouTubePlayerProps {
    ...
    onStateChange?: (event: any) => void; // 🆕 V96.2: Added for parent state sync
...
export const YouTubePlayer = React.forwardRef<any, YouTubePlayerProps>(({
    ...
    onPlayStateChange,
    onTimeUpdate,
    onStateChange, // 🆕 V96.2: Accept from parent
    ...
    useEffect(() => {
        ...
        window.onYouTubeIframeAPIReady = () => {
            console.log('✅ V96.2: YouTube IFrame API Ready');
            setIsAPIReady(true);
        };
    ...
    useEffect(() => {
        ...
        console.log(`🎬 V96.2: Initializing YouTube player: ${videoId}`);
        if (videoStartOffset > 0) {
            console.log(`⏱️ V96.2: Video start offset: ${videoStartOffset}s`);
        }
        ...
        playerRef.current = new window.YT.Player(containerRef.current, {
            ...
            events: {
                onReady: (event: any) => {
                    console.log('✅ V96.2: YouTube player ready');

                    if (videoStartOffset > 0) {
                        console.log(`⏱️ V96.2: Seeking to offset: ${videoStartOffset}s`);
                        event.target.seekTo(videoStartOffset, true);
                    }
                },
                onStateChange: (event: any) => {
                    const state = event.data;
                    console.log(`🎬 V96.2: YouTube state: ${state}`);

                    // 🆕 V96.2: Pass state change to parent (KEY FIX)
                    if (onStateChange) {
                        onStateChange(event);
                    }
                    ...
                    if (state === window.YT.PlayerState.PLAYING && onPlayStateChange) {
                        console.log('▶️ V96.2: YouTube playing - notifying parent');
                        onPlayStateChange(true);
                    } else if (state === window.YT.PlayerState.PAUSED && onPlayStateChange) {
                        console.log('⏸️ V96.2: YouTube paused - notifying parent');
                        onPlayStateChange(false);
                    } else if (state === window.YT.PlayerState.ENDED && onPlayStateChange) {
                        console.log('⏹️ V96.2: YouTube ended - notifying parent');
                        onPlayStateChange(false);
                    }
                },
            },
        ...
        return () => {
            if (playerRef.current && playerRef.current.destroy) {
                console.log('🗑️ V96.2: Destroying YouTube player');
    ...
    useEffect(() => {
        ...
        try {
            ...
            // Only sync when NOT playing AND drift > 10s
            if (!isPlaying && timeDiff > 10.0) {
                isSyncingRef.current = true;
                console.log(`🔁 V96.2: Drift sync (PAUSED, drift=${timeDiff.toFixed(1)}s) → ${youtubeTimeSeconds.toFixed(1)}s`);
                ...
                const checkAndPause = () => {
                    if (playerRef.current && playerRef.current.getPlayerState) {
                        ...
                        if (currentState === window.YT.PlayerState.PLAYING) {
                            ...
                            console.log('⏸️ V96.2: Enforced pause after seek');
            ...
            console.log('⏭️ V96.2: YouTube player not ready for seek');
        }
    }, [currentTime, videoStartOffset, isPlaying]);
    ...
    const handleVariantChange = useCallback((event: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        console.log(`🔄 V96.2: Variant selected: ${variant}`);

        if (videoVariants && videoVariants[variant]) {
            const newVideoId = videoVariants[variant];
            console.log(`🔄 V96.2: Switching to ${variant} video: ${newVideoId}`);

            if (onVariantChange && newVideoId) {
                onVariantChange(newVideoId);
            }
        } else {
            console.warn(`⚠️ V96.2: No video ID found for variant: ${variant}`);
        }
