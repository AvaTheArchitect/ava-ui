...
export class PersonalizationEngine implements BrainModule {
  ...
  /**
   * Legacy method for MaestroBrain compatibility
   * Generates suggestions using the expected interface
   */
  generateSuggestions(context: {
    ...
    try {
      // Extract user ID or use default
      const userId = context.userProfile?.userId || "default-user";
      ...
      // Add guitar-based suggestions
      if (context.guitarAnalysis) {
        suggestions.push(
          this.createGuitarBasedSuggestion(context.guitarAnalysis)
        );
      }

      // Add vocal-based suggestions
      if (context.vocalAnalysis) {
        suggestions.push(
          this.createVocalBasedSuggestion(context.vocalAnalysis)
        );
      }

      // Add default suggestions if none generated
      if (suggestions.length === 0) {
        suggestions.push(...this.generateSkillBasedSuggestions(userProfile));
      }
  ...
  /**
   * Create guitar-based suggestion
   */
  private createGuitarBasedSuggestion(
    guitarAnalysis: any
  ): PersonalizedSuggestion {
    return {
      suggestionId: generateId("guitar-suggestion"),
      type: "technique",
      content: {
        title: "Guitar Technique Improvement",
        description: "Focus on areas identified in your guitar playing",
        instructions: [
          "Practice identified weak areas",
          "Focus on clean execution",
          "Use metronome for timing",
        ],
        materials: ["Guitar", "Metronome"],
        successCriteria: ["Improved accuracy", "Better timing"],
        variations: ["Different tempos", "Various exercises"],
      },
      difficulty: 0.6,
      personalizedReason: "Based on your guitar performance analysis",
      confidence: 0.8,
      expectedEngagement: 0.7,
      estimatedTime: 25,
      prerequisites: [],
  ...
  /**
   * Create vocal-based suggestion
   */
  private createVocalBasedSuggestion(
    vocalAnalysis: any
  ): PersonalizedSuggestion {
    return {
      suggestionId: generateId("vocal-suggestion"),
      type: "technique",
      content: {
        title: "Vocal Technique Enhancement",
        description: "Improve your vocal performance based on analysis",
        instructions: [
          "Practice pitch accuracy exercises",
          "Work on breath control",
          "Focus on tone quality",
        ],
        materials: ["Microphone", "Piano for reference"],
        successCriteria: ["Better pitch accuracy", "Improved tone"],
        variations: ["Different scales", "Various exercises"],
      },
      difficulty: 0.5,
      personalizedReason: "Based on your vocal performance analysis",
      confidence: 0.8,
      expectedEngagement: 0.7,
      estimatedTime: 20,
      prerequisites: [],
  ...
  /**
   * Convert MaestroBrain UserProfile to PersonalizationEngine UserProfile
   */
  convertUserProfile(maestroBrainProfile: any): UserProfile {
    return {
      userId: maestroBrainProfile.userId || "default-user",
      skillLevel: maestroBrainProfile.skillLevel || "beginner",
      preferredGenres: maestroBrainProfile.favoriteGenres || [
        MusicGenre.POP,
        MusicGenre.ROCK,
      ],
      learningGoals: maestroBrainProfile.practiceGoals
        ? maestroBrainProfile.practiceGoals.map((goal: string) => ({
            goalId: generateId("converted-goal"),
            description: goal,
        ...
        : this.generateDefaultLearningGoals(),
      practiceHistory: [],
      personalityProfile: this.generateDefaultPersonalityProfile(),
      learningStyle: this.generateDefaultLearningStyle(),
      preferences: this.generateDefaultPreferences(),
  ...
  /**
   * Alternative generateSuggestions that accepts MaestroBrain UserProfile
   */
  generateSuggestionsWithProfile(context: {
    audioFeatures?: any;
    guitarAnalysis?: any;
    vocalAnalysis?: any;
    userProfile?: any; // MaestroBrain UserProfile type
  }): PersonalizedSuggestion[] {
    try {
      // Convert the userProfile if provided
      let convertedProfile: UserProfile | undefined;
      if (context.userProfile) {
        convertedProfile = this.convertUserProfile(context.userProfile);
      }

      // Use the existing generateSuggestions method
      return this.generateSuggestions({
        ...context,
        userProfile: convertedProfile,
      });
    } catch (error) {
      console.error("❌ Error generating suggestions with profile:", error);
