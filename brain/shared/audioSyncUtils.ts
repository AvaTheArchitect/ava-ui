...
// File: brain/shared/audioSyncUtils.ts
...
// 🎬 Audio Animation Loop Configuration
export interface AnimationLoopConfig {
  fps?: number; // Target frames per second (default: 60)
  onFrame?: (audioData: Float32Array, timestamp: number) => void;
  onSpectralData?: (spectralData: FFTResult) => void;
  onSpectralFeatures?: (features: SpectralFeatures) => void;
  onHarmonicData?: (harmonics: HarmonicAnalysis) => void;
  onOnsetData?: (onsets: OnsetDetection) => void;
  analyzerNode?: AnalyserNode;
  audioContext?: AudioContext;
  enableAdvancedAnalysis?: boolean; // Enable pitch detection, spectral analysis, etc.
}
...
// 🎬 AUDIO ANIMATION LOOP - Visual Synchronization System
// Animation loop state management
let animationFrameId: number | null = null;
let isRunning = false;
let activeAnalyzers: {
  pitchDetector?: AdvancedPitchDetector;
  onsetDetector?: OnsetDetector;
} = {};

export function startAudioAnimationLoop(
  config: AnimationLoopConfig
): () => void {
  if (isRunning) {
    console.warn("🎬 Audio animation loop already running");
    return stopAudioAnimationLoop;
  }
  ...
  if (!analyzerNode || !audioContext) {
    console.error(
      "🚨 AudioContext and AnalyzerNode required for animation loop"
    );
    return () => {};
  }

  // Initialize advanced analyzers if enabled
  if (enableAdvancedAnalysis) {
    activeAnalyzers.pitchDetector = new AdvancedPitchDetector(
      audioContext.sampleRate
    );
    activeAnalyzers.onsetDetector = new OnsetDetector();
  }
  ...
  const animate = (timestamp: number) => {
    ...
    if (timestamp - lastFrameTime >= targetFrameTime) {
      try {
        // Get current audio data from Web Audio API
        analyzerNode.getFloatTimeDomainData(audioData);
        analyzerNode.getByteFrequencyData(frequencyData);

        // Convert to spectral data if callback provided
        if (onSpectralData) {
          const spectralData: FFTResult = {
            frequencies: new Float32Array(frequencyData.length),
            magnitudes: new Float32Array(frequencyData.length),
            phases: new Float32Array(frequencyData.length),
            sampleRate: audioContext.sampleRate,
            binSize: audioContext.sampleRate / (bufferLength * 2),
          };

          // Convert frequency data to proper format
          for (let i = 0; i < frequencyData.length; i++) {
            spectralData.magnitudes[i] = frequencyData[i] / 255.0;
            spectralData.frequencies[i] =
              (i * audioContext.sampleRate) / (bufferLength * 2);
            spectralData.phases[i] = 0; // Phase info not available from basic AnalyzerNode
          }
          ...
          if (enableAdvancedAnalysis) {
            ...
            if (onSpectralFeatures) {
              const features =
                SpectralAnalyzer.computeSpectralFeatures(spectralData);
              onSpectralFeatures(features);
            }

            // Pitch and harmonic analysis
            if (
              (onHarmonicData || onOnsetData) &&
              activeAnalyzers.pitchDetector
            ) {
              const pitchResult =
                activeAnalyzers.pitchDetector.detectPitch(audioData);

              if (onHarmonicData && pitchResult.frequency > 0) {
                const harmonicData = HarmonicAnalyzer.analyzeHarmonics(
                  spectralData,
                  pitchResult.frequency
                );
                onHarmonicData(harmonicData);
              }
            }

            // Onset detection
            if (onOnsetData && activeAnalyzers.onsetDetector) {
              const onsetData = activeAnalyzers.onsetDetector.detectOnsets(
                audioData,
                audioContext.sampleRate,
                timestamp
              );
              onOnsetData(onsetData);
            }
          }
        }

        // Call frame callback with raw audio data
        if (onFrame) {
          onFrame(audioData, timestamp);
        }
      } catch (error) {
        console.error("🚨 Error in audio animation loop:", error);
      }
  ...
  console.log("🎬 Starting audio animation loop at", fps, "FPS");
  isRunning = true;
  animationFrameId = requestAnimationFrame(animate);
...
export function stopAudioAnimationLoop(): void {
  if (!isRunning) {
    console.warn("🎬 Audio animation loop not running");
    return;
  }

  console.log("🛑 Stopping audio animation loop");
  isRunning = false;
...
// 📦 BASIC UTILITY FUNCTIONS - merged from old audioUtils.ts
...
export const base64ToAudioBuffer = (base64: string): ArrayBuffer => {
...
export const validateAudioFeatures = (features: any): boolean => {
  return !!(
    features &&
    typeof features.pitch === "number" &&
    typeof features.volume === "number" &&
    Array.isArray(features.frequency) &&
    Array.isArray(features.amplitude)
  );
};

// Guitar-specific frequency ranges (useful for ChordAnalyzer)
export const GUITAR_FREQUENCY_RANGE = {
  LOW_E: 82.41, // E2
  HIGH_E: 329.63, // E4
  MIN: 80, // Slightly below low E
  MAX: 2000, // Upper harmonics range
};

export const isGuitarFrequency = (frequency: number): boolean => {
  return (
    frequency >= GUITAR_FREQUENCY_RANGE.MIN &&
    frequency <= GUITAR_FREQUENCY_RANGE.MAX
  );
};

// Export everything including the merged functions
export default {
  ...
  // Merged basic utilities
  audioBufferToBase64,
  base64ToAudioBuffer,
  validateAudioFeatures,
  GUITAR_FREQUENCY_RANGE,
  isGuitarFrequency,
};
